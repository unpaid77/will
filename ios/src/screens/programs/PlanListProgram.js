...
const PlanListProgram = (props) => {
  ...
  const {programPlanData} = props;

  useEffect(() => {
    if (props.programPlanData) {
      props.programPlanData.duration.forEach((e) => {
        if (e.is_selected == true) {
          setgender_id(parseInt(e.gender));
        }
      });
    }
    const unsubscribe = navigation.addListener('focus', () => {
      ...
      setgender_id(1);
      setweekNumber(
        parseInt(props.programPlanData.duration[0].Week.replace('Week', '')),
      );
  ...
  if (props.programPlanData) {
    let programData = programPlanData;
    return (
      <>
        <Header />
        <ScrollView
          ...
          <View
            ...
            <TouchableOpacity
              ...
              onPress={() => {
                selectGender(gender_id == 2 ? 1 : 2);
              }}>
              <Image
                source={gender_id == 2 ? REC_SELECTED : REC}
            ...
            <TouchableOpacity
              style={{flexDirection: 'row'}}
              onPress={() => {
                selectGender(gender_id == 3 ? 1 : 3);
              }}>
              <Image
                source={gender_id == 3 ? REC_SELECTED : REC}
                style={{marginRight: 5, width: 10, height: 10, marginTop: 5}}
              />
